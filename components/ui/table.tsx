...
/** A ledger table. Mono body, hairline rows, tabular figures. */
const Table = React.forwardRef<HTMLTableElement, React.HTMLAttributes<HTMLTableElement>>(
  ({ className, ...props }, ref) => (
    <div className="w-full overflow-x-auto">
      <table ref={ref} className={cn('ledger w-full caption-bottom text-sm', className)} {...props} />
    </div>
  ),
)
...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        'border-b border-border/70 transition-colors data-[state=selected]:bg-accent hover:bg-accent/40',
        className,
      )}
      {...props}
    />
